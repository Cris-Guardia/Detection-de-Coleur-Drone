...
while True:
    ...
    elif key == ord('a') and not movement:  # Comencer deplacement avec 'a'
        movement = True
        
        while movement and deplacement < 100:
            key = cv2.waitKey(1) & 0xFF #Deuxieme varification de clavier pour arreter o non

            if deplacement < 100:
                tello.send_rc_control(direction, 0, 0, 0)
                time.sleep(0.1)
                deplacement += 5
                temps_vol += 0.1
                clear()
                print("Mouvement")
                print(f"{deplacement} cm. temps {temps_vol:.2f}s")
              
            elif deplacement < 200 and key == ord('a'):
                tello.send_rc_control(direction, 0, 0, 0)
                time.sleep(0.1)
                deplacement += 5
                temps_vol += 0.1
                ...
                print("Mouvement")
                print(f"key {key}")
                print(f"{deplacement} cm. ///// Temps vol {temps_vol:.2f}s")

            else:
                movement = False

        if not movement :
            while deplacement < 100:
                tello.send_rc_control(direction, 0, 0, 0)
                time.sleep(0.1)
                deplacement += 5
                temps_vol += 0.1
                clear()
                print("Mouvement")
                print(f"{deplacement} cm. ///// Temps vol {temps_vol:.2f}")

        tello.send_rc_control(0, 0, 0, 0) 
        movement = False
        print(f"Finalise: {deplacement} cm deplacées. ///// Temps vol {temps_vol:.2f}s")
        deplacement = 0
        temps_vol = 0
        direction *= -1

        tello.send_rc_control(0, 0, 0, 0)
        print(f"Distance minimale: {deplacement} cm. ///// Temps vol {temps_vol:.2f}s")
